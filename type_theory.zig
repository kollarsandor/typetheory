                ...
                .in_struct => {
                    if (brace_depth == 1 and (std.mem.startsWith(u8, line, "pub fn ") or std.mem.startsWith(u8, line, "fn "))) {
                        if (self.matchFuncDecl(line)) |method_name| {
                            ...
                            method_decl.source_text = source;
                            if (current_decl) |cd| {
...
"), "0");
    ...
    fn buildArithmeticRewrite(self: *SemanticProofEngine, builder: *ProofBuilder, node: std.zig.Ast.Node.Index) !SemanticArithmeticRewrite {
        ...
        if (self.isAddLikeTag(tag)) {
            const lhs_node = datas[node].lhs;
            const rhs_node = datas[node].rhs;
            const lhs_expr = try self.ownString(self.nodeText(lhs_node));
            const rhs_expr = try self.ownString(self.nodeText(rhs_node));
            if (self.isZeroNode(rhs_node)) {
                const base = try builder.raw(try self.allocOwnedFmt("Nat.add_zero ({s})", .{lhs_expr}));
                const proof = try builder.trans(try builder.refl(expr_wrapped), base);
                return .{ .expr = expr, .target = lhs_expr, .proof = proof };
            }
            if (self.isZeroNode(lhs_node)) {
                const base = try builder.raw(try self.allocOwnedFmt("Nat.zero_add ({s})", .{rhs_expr}));
                const proof = try builder.trans(try builder.refl(expr_wrapped), base);
                return .{ .expr = expr, .target = rhs_expr, .proof = proof };
            }
            if (self.isAddLikeTag(tags[lhs_node])) {
                const a = try self.ownString(self.nodeText(datas[lhs_node].lhs));
                const b = try self.ownString(self.nodeText(datas[lhs_node].rhs));
                const base = try builder.raw(try self.allocOwnedFmt("Nat.add_assoc ({s}) ({s}) ({s})", .{ a, b, rhs_expr }));
                const proof = try builder.trans(try builder.refl(expr_wrapped), base);
                const target = try self.allocOwnedFmt("({s} + ({s} + {s}))", .{ a, b, rhs_expr });
                return .{ .expr = expr, .target = target, .proof = proof };
            }
            if (self.isAddLikeTag(tags[rhs_node])) {
                const b = try self.ownString(self.nodeText(datas[rhs_node].lhs));
                const c = try self.ownString(self.nodeText(datas[rhs_node].rhs));
                if (self.isZeroNode(datas[rhs_node].rhs)) {
                    const child = try self.buildArithmeticRewrite(builder, rhs_node);
                    const lifted = try builder.congrArg(try self.allocOwnedFmt("(fun t => ({s}) + t)", .{lhs_expr}), child.proof);
                    const proof = try builder.trans(try builder.refl(expr_wrapped), lifted);
                    const target = try self.allocOwnedFmt("({s} + {s})", .{ lhs_expr, child.target });
                    return .{ .expr = expr, .target = target, .proof = proof };
                }
                const base = try builder.raw(try self.allocOwnedFmt("Nat.add_assoc ({s}) ({s}) ({s})", .{ lhs_expr, b, c }));
                const proof = try builder.trans(try builder.refl(expr_wrapped), try builder.symm(base));
                const target = try self.allocOwnedFmt("(({s} + {s}) + {s})", .{ lhs_expr, b, c });
                return .{ .expr = expr, .target = target, .proof = proof };
            }
        }
        if (self.isSubLikeTag(tag)) {
            const lhs_node = datas[node].lhs;
            const rhs_node = datas[node].rhs;
            const lhs_expr = try self.ownString(self.nodeText(lhs_node));
            if (self.isZeroNode(rhs_node)) {
                const base = try builder.raw(try self.allocOwnedFmt("Nat.sub_zero ({s})", .{lhs_expr}));
                const proof = try builder.trans(try builder.refl(expr_wrapped), base);
                return .{ .expr = expr, .target = lhs_expr, .proof = proof };
            }
        }
        return .{ .expr = expr, .target = expr, .proof = try builder.refl(expr_wrapped) };
    }
    ...
    fn emitPreamble(self: *SemanticProofEngine) !void {
        ...
        try self.nl();
        try self.wl("inductive MemoryCell where");
        ...
        try self.wl("deriving DecidableEq");
        try self.nl();
        ...
        try self.wl("  floatSlots : MemoryCell -> Float");
        try self.nl();
        ...
        try self.nl();
        try self.wl("def writeNat : MemoryState -> MemoryCell -> Nat -> MemoryState :=");
        ...
        try self.nl();
        try self.wl("def writeInt : MemoryState -> MemoryCell -> Int -> MemoryState :=");
        try self.wl("  fun sigma key value =>");
        ...
        try self.nl();
        try self.wl("def writeBool : MemoryState -> MemoryCell -> Bool -> MemoryState :=");
        try self.wl("  fun sigma key value =>");
        ...
        try self.nl();
        try self.wl("def writeFloat : MemoryState -> MemoryCell -> Float -> MemoryState :=");
        try self.wl("  fun sigma key value =>");
        ...
        try self.nl();
        try self.wl("theorem writeNat_same : (sigma : MemoryState) -> (key : MemoryCell) -> (value : Nat) -> Eq ((writeNat sigma key value).natSlots key) value :=");
        try self.wl("  fun sigma key value => Eq.refl value");
        try self.nl();
        try self.wl("theorem writeInt_same : (sigma : MemoryState) -> (key : MemoryCell) -> (value : Int) -> Eq ((writeInt sigma key value).intSlots key) value :=");
        try self.wl("  fun sigma key value => Eq.refl value");
        try self.nl();
        try self.wl("theorem writeBool_same : (sigma : MemoryState) -> (key : MemoryCell) -> (value : Bool) -> Eq ((writeBool sigma key value).boolSlots key) value :=");
        try self.wl("  fun sigma key value => Eq.refl value");
        try self.nl();
        try self.wl("theorem writeFloat_same : (sigma : MemoryState) -> (key : MemoryCell) -> (value : Float) -> Eq ((writeFloat sigma key value).floatSlots key) value :=");
        try self.wl("  fun sigma key value => Eq.refl value");
        try self.nl();
    }
    ...
    fn emitArithmeticProof(self: *SemanticProofEngine, function: *const SemanticFunctionIR) !void {
        ...
        for (function.obligations.items) |obligation| {
            ...
            try self.wfmt("_arith_{d} : (sigma : MemoryState)", .{arithmetic_index});
...
pub const LeanProofGenerator = struct {
    ...
    pub fn generate(self: *LeanProofGenerator, decls: []const *ZigSourceParser.ParsedDecl) ![]const u8 {
        ...
        return try self.generateFromSource(decls[0].source_text);
    }
